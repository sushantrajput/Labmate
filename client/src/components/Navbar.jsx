...
const Navbar = () => {
  ...
  const AuthButtons = () => (
    <div className="hidden md:flex items-center space-x-3">
      <a
        ...
        className="flex items-center gap-2 bg-teal-50 hover:bg-teal-100 text-primary px-4 py-2 rounded-full border border-teal-100 font-medium"
      >
    ...
    <>
      {navLinks.map(({ path, label, icon: Icon }) => (
        <NavLink
          ...
          className={({ isActive }) =>
            `${mobile
              ? `py-2 px-4 rounded-lg flex items-center gap-3 ${isActive ? 'bg-teal-50 text-primary' : 'text-gray-700 hover:bg-teal-50'}`
              : `py-1 flex items-center gap-1.5 font-medium relative group transition ${isActive ? 'text-primary' : 'text-gray-700 hover:text-primary'}`
            }`
          }
        >
          <Icon className={`${mobile ? "text-lg" : "text-lg text-primary"}`} />
          {label}
          {!mobile && <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary group-hover:w-full transition-all duration-300" />}
        </NavLink>
      ))}
    </>
  );
      ...
      <div className="min-w-48 bg-white rounded-lg shadow-xl flex flex-col p-4 gap-3 border border-gray-100">
        {userMenu.map(({ label, icon: Icon, path, action, isLogout }) => (
          <p
            ...
            className={`cursor-pointer flex items-center gap-2 hover:text-primary transition ${isLogout ? 'text-red-500 hover:text-red-600' : ''}`}
          >
            <Icon className={`${isLogout ? 'text-red-500' : 'text-primary'}`} />
            {label}
          </p>
        ))}
      </div>
  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      {/* Top Info Bar - Updated to Teal Gradient */}
      <div className="hidden md:block bg-gradient-to-r from-teal-900 to-teal-600 text-white text-sm py-2 px-4">
      ...
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center cursor-pointer" onClick={() => navigate("/")}>
          <img src={assets.logo} alt="LabMate Logo" className="h-10 mr-2" />
          <span className="text-2xl font-bold text-primary">LabMate</span>
        </div>
        ...
        <div className="flex items-center space-x-6">
            ...
            >
              <div className="relative">
                <img
                  ...
                  alt="Profile"
                  className="rounded-full w-10 h-10 object-cover border-2 border-teal-100"
                />
          ...
          {/* Mobile Toggle */}
          <button className="md:hidden text-primary" onClick={() => setShowMenu(true)}>
            <FaBars size={24} />
          </button>
        </div>
      </div>
      ...
      <div className={`md:hidden fixed top-0 left-0 w-72 h-full z-50 bg-white transform transition-transform duration-300 shadow-2xl ${showMenu ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <div className="flex items-center cursor-pointer" onClick={() => { navigate("/"); setShowMenu(false); }}>
            <img src={assets.logo} alt="LabMate Logo" className="h-8 mr-2" />
            <span className="text-xl font-bold text-primary">LabMate</span>
          </div>
            ...
            {token && userData ? (
                ...
                <div className="space-y-3">
                  {userMenu.map(({ label, icon: Icon, path, action, isLogout }) => (
                    <button
                      ...
                      className={`w-full text-left py-2 px-4 rounded-lg flex items-center gap-3 hover:bg-teal-50 ${isLogout ? 'text-red-500' : ''}`}
                    >
                      <Icon className={`${isLogout ? 'text-red-500' : 'text-primary'}`} />
                      {label}
                    </button>
              ...
              <div className="space-y-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-2 bg-teal-50 hover:bg-teal-100 text-primary px-6 py-3 rounded-full border border-teal-100 font-medium"
                >
                  <FaUserMd /><span>Staff Login</span><FaExternalLinkAlt className="text-xs" />
                </a>
                <button
